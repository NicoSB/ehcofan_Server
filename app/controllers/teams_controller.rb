class TeamsController < ApplicationController
	...
	def index
		if(params[:mode] != nil && params[:mode] == "control")
			@teams = Team.all
		elsif(params[:competition] != nil && params[:competition] != "")
			...
			@teams = Team.order("teams.group ASC, wins*3 + ot_wins*2 + ot_losses DESC, goals_for - goals_against DESC , goals_for DESC")
			render :json => @teams, :except=>[:updated_at, :created_at]
		end
	end
	...
	private
		def team_params
			params.require(:team).permit(:name, :competition, :wins, :ot_wins, :ot_losses, :losses, :goals_for, :goals_against, :group, :active)	
		end
end
